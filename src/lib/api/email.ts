...
export async function sendPasswordResetEmail(data: {
  ...
  await resend.emails.send({
    ...
    to: data.to,
    subject: 'Reset your MabEcare Admin password',
    html: `
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">

        <div style="background: linear-gradient(135deg, #1a0010 0%, #2d0020 60%, #0d0d0d 100%); padding: 36px 32px; position: relative;">
          <div style="position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, #ff1493, #ff69b4);"></div>
          <h1 style="color: #ffffff; font-size: 22px; font-weight: 700; margin: 0;">Password Reset Requested</h1>
          <p style="color: rgba(255,255,255,0.5); font-size: 13px; margin: 8px 0 0;">MabEcare Admin Dashboard</p>
        </div>

        <div style="padding: 32px;">
          <p style="color: #374151; font-size: 15px; line-height: 1.7; margin: 0 0 20px;">
            We received a request to reset the password for your MabEcare admin account. Click the button below to set a new password.
          </p>

          <div style="text-align: center; margin: 0 0 24px;">
            <a href="${data.resetUrl}" style="display: inline-block; background: linear-gradient(135deg, #ff1493, #e0006a); color: #ffffff; font-size: 14px; font-weight: 700; padding: 14px 36px; border-radius: 10px; text-decoration: none; box-shadow: 0 4px 14px rgba(255,20,147,0.35);">
              Reset Password
            </a>
          </div>
